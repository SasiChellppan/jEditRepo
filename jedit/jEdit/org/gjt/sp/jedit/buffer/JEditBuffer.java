...
public class JEditBuffer {
	...
	public boolean indentLine(int lineIndex, boolean canDecreaseIndent) {
		...
		int prevLineIndent = (prevLineIndex == -1) ? 0 : StandardUtilities
		...
		try {
			...
			String prevIndentString = (prevLineIndex >= 0) ? StandardUtilities
					.getIndentString(getLineText(prevLineIndex)) : null;
			String indentString;
			if (prevIndentString == null) {
				indentString = StandardUtilities.createWhiteSpace(idealIndent,
				...
				indentString = prevIndentString
						+ StandardUtilities
								.createWhiteSpace(idealIndent - prevLineIndent,
										getBooleanProperty("noTabs") ? 0
												: getTabSize(), prevLineIndent);
	 ...
	 *            The line number
	 * @param prevLineIndex
	 *            The index of the previous non-empty line
	 * @param oldIndent
	 *            The indent width of the previous line (or 0)
	 */
			...
			for (int i = firstInvalidFoldLevel; i <= line; i++) {
				...
				if (newFoldLevel != lineMgr.getFoldLevel(i)) {
					...
					if (i == firstInvalidFoldLevel) {
						List<Integer> precedingFoldLevels = foldHandler
								.getPrecedingFoldLevels(this, i, seg,
										newFoldLevel);
	...
	// {{{ getUndoId() method
	/**
	 * Returns an object that identifies the undo operation to which the current
	 * content change belongs. This method can be used by buffer listeners
	 * during content changes (contentInserted/contentRemoved) to find out which
	 * content changes belong to the same "undo" operation. The same undoId
	 * object will be returned for all content changes belonging to the same
	 * undo operation. Only the identity of the undoId can be used, by comparing
	 * it with a previously-returned undoId using "==".
	 * 
